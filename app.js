        ...
        Viewpet.innerHTML = `
          <div class="bg-white rounded-lg shadow-lg p-6 hover:shadow-xl transition duration-300 ease-out min-w-full">
            ...
            <div class="flex justify-around">
              ...
              <button onclick= "adoptPet('${pet.petId}', this)" class="text-Primary-Btn bg-green-200 px-4 py-2 rounded-md">Adopt</button>
              <button onclick="loadDetails('${pet.petId}')" class="text-Primary-Btn bg-green-200 px-4 py-2 rounded-md">Details</button>
            </div>
          </div>`;
    ...
    detailsContainer.innerHTML = `
          ...
          <div class="bg-white rounded-lg p-6 hover: transition duration-300 ease-out w-full h-full">
            ...
            <div class="grid grid-cols-2">
                <p class="text-gray-800 text-sm mb-2">Breed: ${petData.breed || "Not Available"}</p>
                <p class="text-gray-800 text-sm mb-2">Birth: ${petData.date_of_birth || "Not Available"}</p>
                <p class="text-gray-800 text-sm mb-2">Gender: ${petData.gender}</p>
                <p class="text-gray-800 text-sm mb-2">Price: ${petData.price || "Not Available"}</p>
                <p class="text-gray-800 text-sm mb-2">Vaccinated status: ${petData.vaccinated_status || "Not Available"}</p>
            </div>
    ...
    if (!pets || pets.length === 0) {
        ...
        pets.forEach((pet) => {
          ...
          <div class="bg-white rounded-lg shadow-lg p-6 hover:shadow-xl transition duration-300 ease-out min-w-full">
            <img src="${pet.image}" alt="${pet.pet_name}" class="rounded-lg mb-4">
            <h3 class="text-xl font-semibold mb-2 ">${pet.pet_name}</h3>
            <p class="text-gray-800 text-sm mb-2">Breed: ${pet.breed || "Not Available"}</p>
            <p class="text-gray-800 text-sm mb-2">Birth: ${pet.date_of_birth || "Not Available"}</p>
            <p class="text-gray-800 text-sm mb-2">Gender: ${pet.gender}</p>
            <p class="text-gray-800 text-sm mb-2">Price: $${pet.price || "Not Available"}</p>
            <div class="flex justify-around">
              <button id="" class="like-btn bg-gray-200 px-4 py-2 rounded-md"><i class="fa-regular fa-thumbs-up"></i></button>
              <button onclick= "adoptPet('${pet.petId}', this)" class="text-Primary-Btn bg-green-200 px-4 py-2 rounded-md">Adopt</button>
             <button onclick="loadDetails('${pet.petId}')" class="text-Primary-Btn bg-green-200 px-4 py-2 rounded-md">Details</button>
